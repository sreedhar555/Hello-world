...
namespace NOSNew.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        public ActionResult NosNewLogin(string returnUrl)
        {
            ViewBag.ReturnUrl = returnUrl;
            // GetOverallEvaluationReport(1);
            return View();
        }
        ...
        public ActionResult Logout()
        {
            ...
            return RedirectToAction("NosNewLogin", "Account");
          

        }

        [NonAction]
        public int InsertLogOnAudit(string username, string password, string invaliduser)
        {
            int lngReturn = 0;
            String myHost = "";
            ...
            LogonAuditBL objBl = new LogonAuditBL();
            LogonAuditDO objDO = new LogonAuditDO();
            objDO.IPAddress = strClientIP;
            objDO.URL = password;// GetUrl();
            objDO.LoginAttempt = Convert.ToInt16(ViewData["count"]);
            ...
            if (flag == true)
            {
                ...
                objDO.LoginStatus = true;
                objDO.InstanceId = Convert.ToInt32(Session["InstanceId"].ToString());
                objDO.UserName = Session["UserName"].ToString();
                objDO.CreatedBy = Convert.ToInt32(Session["UserId"].ToString());
            }
            else
            {
                objDO.LoginStatus = false;
                objDO.InstanceId = 0;
                objDO.UserName = username;
                objDO.CreatedBy = 1;
            }

            lngReturn = objBl.SaveLogonAudit(objDO);
